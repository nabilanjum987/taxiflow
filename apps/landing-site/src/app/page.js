...
const bookingSchema = zod_2.z.object({
    phone: zod_2.z.string().min(7, 'Enter a valid phone number'),
    pickupAddress: zod_2.z.string().min(1, 'Pickup address required'),
    dropoffAddress: zod_2.z.string().min(1, 'Drop-off address required'),
    vehicleType: zod_2.z.enum(['STANDARD', 'EXECUTIVE', 'MPV', 'WAV']),
    scheduledAt: zod_2.z.string().optional(),
});
const VEHICLE_TYPES = [
...
function LandingPage() {
    const [bookingStep, setBookingStep] = (0, react_1.useState)('form');
    ...
    const apiWithTenant = axios_1.default.create({ baseURL: API, headers: { 'x-tenant-id': TENANT_ID } });
    ...
    return (<div className="min-h-screen bg-white">
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-amber-500 flex items-center justify-center"><lucide_react_1.Car size={16} className="text-white"/></div>
            <span className="font-bold text-gray-900 text-lg">CityRide</span>
          </div>
          <div className="hidden md:flex items-center gap-6">
            {['Services', 'About', 'Contact'].map(item => (<a key={item} href={`#${item.toLowerCase()}`} className="text-sm text-gray-600 hover:text-gray-900 transition-colors">{item}</a>))}
          </div>
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-20 grid md:grid-cols-2 gap-12 items-center">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/20 text-amber-400 text-xs font-medium mb-6 border border-amber-500/20">
              <lucide_react_1.Zap size={12}/> Available 24/7
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <a href="#book" className="flex items-center gap-2 px-6 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-white font-semibold transition-all">
                Book a Ride <lucide_react_1.ChevronRight size={18}/>
              </a>
              <a href="#" className="flex items-center gap-2 px-6 py-3 rounded-xl border border-white/20 hover:bg-white/10 text-white font-medium transition-colors">
                <lucide_react_1.Smartphone size={18}/> Get the App
              </a>
            </div>
            <div className="flex items-center gap-6 mt-10">
              {[['50K+', 'Happy Riders'], ['1,200+', 'Expert Drivers'], ['4.9★', 'Average Rating']].map(([val, lbl]) => (<div key={lbl}><div className="text-xl font-bold text-amber-400">{val}</div><div className="text-gray-400 text-xs">{lbl}</div></div>))}
            </div>
          </div>
          ...
          <div id="book" className="bg-white rounded-2xl p-6 shadow-2xl">
            {bookingStep === 'form' && (<>
                ...
                <form onSubmit={handleSubmit(onSubmitBooking)} className="space-y-4">
                  <div>
                    ...
                    <input {...register('phone')} placeholder="+44 7700 000000" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all"/>
                    {errors.phone && <p className="text-red-500 text-xs mt-1">{errors.phone.message}</p>}
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1.5 flex items-center gap-1"><lucide_react_1.MapPin size={11} className="text-emerald-500"/> Pickup</label>
                    <input {...register('pickupAddress')} placeholder="Enter pickup address" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all"/>
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1.5 flex items-center gap-1"><lucide_react_1.MapPin size={11} className="text-red-500"/> Drop-off</label>
                    <input {...register('dropoffAddress')} placeholder="Enter drop-off address" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all"/>
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-2">Vehicle Type</label>
                    <div className="grid grid-cols-2 gap-2">
                      {VEHICLE_TYPES.map(v => (<button key={v.value} type="button" onClick={() => setSelectedVehicle(v.value)} className={`p-3 rounded-xl border text-left transition-all ${selectedVehicle === v.value ? 'border-amber-400 bg-amber-50' : 'border-gray-200 hover:border-gray-300'}`}>
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1.5 flex items-center gap-1"><lucide_react_1.Clock size={11}/> Schedule (optional)</label>
                    <input {...register('scheduledAt')} type="datetime-local" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-amber-400 transition-all"/>
                  </div>
                  {error && <p className="text-red-500 text-xs bg-red-50 p-3 rounded-lg">{error}</p>}
                  <button type="submit" disabled={submitting} className="w-full py-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm transition-all disabled:opacity-60 flex items-center justify-center gap-2">
                    {submitting ? 'Sending OTP...' : 'Continue →'}
                  </button>
                </form>
              </>)}

            {bookingStep === 'otp' && (<div className="py-4">
                ...
                <input value={otp} onChange={e => setOtp(e.target.value)} maxLength={6} placeholder="000000" className="w-full px-4 py-3 rounded-xl border border-gray-200 text-center text-2xl font-mono tracking-[0.5em] outline-none focus:border-amber-400 transition-all mb-4"/>
                {error && <p className="text-red-500 text-xs bg-red-50 p-3 rounded-lg mb-4">{error}</p>}
                <button onClick={verifyAndBook} disabled={submitting || otp.length < 6} className="w-full py-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm transition-all disabled:opacity-60">
            ...
            {bookingStep === 'confirmed' && (<div className="py-8 text-center">
                <div className="w-16 h-16 rounded-2xl bg-emerald-100 flex items-center justify-center mx-auto mb-4">
                  <lucide_react_1.Check size={32} className="text-emerald-600"/>
                </div>
      ...
      <section id="services" className="py-20 bg-gray-50">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
            { icon: lucide_react_1.Clock, title: 'Always On Time', desc: 'Real-time GPS tracking so you always know where your driver is', color: 'bg-blue-100 text-blue-600' },
            { icon: lucide_react_1.Shield, title: 'Safe & Vetted', desc: 'All drivers are DBS checked, licensed, and professionally trained', color: 'bg-emerald-100 text-emerald-600' },
            { icon: lucide_react_1.Star, title: 'Top Rated', desc: '4.9 stars from over 50,000 happy passengers across the city', color: 'bg-amber-100 text-amber-600' },
        ].map(({ icon: Icon, title, desc, color }) => (<div key={title} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            {VEHICLE_TYPES.map(v => (<div key={v.value} className="p-6 rounded-2xl border border-gray-100 hover:border-amber-200 hover:shadow-md transition-all text-center group">
      ...
      <section className="py-20 bg-gray-900 text-white">
        <div className="max-w-6xl mx-auto px-4 text-center">
          <lucide_react_1.Smartphone size={40} className="text-amber-400 mx-auto mb-5"/>
          ...
          <div className="flex justify-center gap-4 flex-wrap">
            {['App Store', 'Google Play'].map(store => (<button key={store} className="flex items-center gap-3 px-6 py-3 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10 transition-colors">
                <lucide_react_1.Phone size={18} className="text-amber-400"/>
      ...
      <section id="contact" className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6 text-center">
            {[
            { icon: lucide_react_1.Phone, label: 'Call Us', value: '0800 123 4567', link: 'tel:08001234567', color: 'text-emerald-600 bg-emerald-50' },
            { icon: lucide_react_1.Mail, label: 'Email Us', value: 'hello@cityride.com', link: 'mailto:hello@cityride.com', color: 'text-blue-600 bg-blue-50' },
            { icon: lucide_react_1.Clock, label: 'Operating Hours', value: '24/7 — Always Open', link: null, color: 'text-amber-600 bg-amber-50' },
        ].map(({ icon: Icon, label, value, link, color }) => (<div key={label} className="p-6 rounded-2xl border border-gray-100">
      ...
      <footer className="border-t border-gray-100 py-8 bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-lg bg-amber-500 flex items-center justify-center"><lucide_react_1.Car size={12} className="text-white"/></div>
