...
import { Car, Users, TrendingUp, Clock, CheckCircle2, XCircle, Star, AlertCircle } from 'lucide-react';
import api from '../lib/api';
import { formatCurrency } from '@taxiflow/shared-utils';
...
export default function DashboardPage() {
  const { tenant } = useAuthStore((s) => ({ tenant: s.user }));
  ...
  const { data: revenue } = useQuery({
    queryKey: ['revenue-7d'],
    queryFn: () => api.get('/reports/revenue?days=7').then(r => r.data.data),
  });

  const { data: recentBookings } = useQuery({
    queryKey: ['recent-bookings'],
    queryFn: () => api.get('/bookings?limit=6').then(r => r.data.data),
  });

  const { data: pendingDrivers } = useQuery({
    queryKey: ['pending-drivers'],
    queryFn: () => api.get('/drivers?status=PENDING_APPROVAL&limit=5').then(r => r.data.data),
  });
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          label="Today's Bookings" icon={Car} color="bg-amber-500"
          value={statsLoading ? '—' : stats?.today?.bookings ?? 0}
          sub="All statuses"
        />
        <StatCard
          label="Today's Revenue" icon={TrendingUp} color="bg-emerald-500"
          value={statsLoading ? '—' : formatCurrency(stats?.today?.revenue ?? 0, currency)}
          trend="+12%"
        />
        <StatCard
          label="Active Drivers" icon={Users} color="bg-blue-500"
          value={statsLoading ? '—' : stats?.drivers?.active ?? 0}
          sub={`${stats?.drivers?.online ?? 0} online now`}
        />
        <StatCard
          label="Avg Rating" icon={Star} color="bg-violet-500"
          value={statsLoading ? '—' : `${stats?.averageRating ?? 0} ★`}
          sub="Passenger ratings"
        />
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Completed', value: stats?.weeklyBookings?.COMPLETED ?? 0, icon: CheckCircle2, cls: 'text-emerald-500' },
          { label: 'Cancelled', value: stats?.weeklyBookings?.CANCELLED ?? 0, icon: XCircle, cls: 'text-red-500' },
          { label: 'In Progress', value: stats?.weeklyBookings?.IN_PROGRESS ?? 0, icon: Clock, cls: 'text-blue-500' },
          { label: 'Pending Approval', value: stats?.drivers?.pendingApproval ?? 0, icon: AlertCircle, cls: 'text-amber-500' },
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={revenue?.byDay ?? []}>
              ...
              <Area type="monotone" dataKey="revenue" stroke="#f59e0b" strokeWidth={2}
                fill="url(#revGrad)" dot={{ fill: '#f59e0b', strokeWidth: 0, r: 3 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/drivers?status=PENDING_APPROVAL" className="text-xs text-amber-600 hover:underline">View all</Link>
          </div>
          {pendingDrivers?.length === 0 ? (
            <div className="text-center py-8 text-gray-400 text-sm">No pending approvals</div>
          ) : (
            <div className="space-y-3">
              {(pendingDrivers ?? []).map((d: { id: string; user: { firstName: string; lastName: string; phone: string; avatarUrl?: string } }) => (
      ...
      <div className="card">
        <div className="flex items-center justify-between p-5 border-b border-gray-50">
          ...
          <Link to="/bookings" className="text-xs text-amber-600 hover:underline">View all</Link>
        </div>
