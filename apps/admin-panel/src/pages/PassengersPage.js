...
exports.default = PassengersPage;
...
function PassengersPage() {
    ...
    const { data, isLoading } = (0, react_query_1.useQuery)({
        ...
        queryFn: () => api_1.default.get(`/passengers?page=${page}&limit=15${search ? `&search=${search}` : ''}`).then(r => r.data),
    ...
    return (<div className="space-y-5 max-w-7xl mx-auto">
      ...
      <div className="card p-4">
        <div className="relative max-w-sm">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search passengers..." className="input pl-9 py-2 text-sm"/>
        </div>
      </div>
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="border-b border-gray-100 bg-gray-50/50">
            <tr>{['Passenger', 'Phone', 'Email', 'Trips', 'Rating', 'Joined'].map(h => (<th key={h} className="text-left text-xs text-gray-400 font-medium px-5 py-3.5">{h}</th>))}</tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            ...
            : passengers.map((p) => (<tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
...
//# sourceMappingURL=PassengersPage.js.map